...
# ---------------------------------------------------------
# CONEXIÓN A BASE DE DATOS EN LA NUBE (TURSO / LIBSQL-CLIENT)
# ---------------------------------------------------------
def execute_query(query, params=()):
    url = st.secrets["TURSO_DATABASE_URL"]
    token = st.secrets["TURSO_AUTH_TOKEN"]
    # Convertir URL a HTTPS para cliente de red liviano
    if url.startswith("libsql://"):
        url = url.replace("libsql://", "https://")
    
    with libsql_client.create_client_sync(url, auth_token=token) as client:
    ...
    if st.session_state.socio_autenticado is None:
        ...
        # --- SECCIÓN: HISTORIAL ---
            ...
            if not df_hist.empty:
                ...
                st.info("Sin movimientos en el historial.")ime import datetime, timedelta
import pypdf
import re
import io
import urllib.parse
