...
# ---------------------------------------------------------
# CONEXIÓN A BASE DE DATOS LOCAL (SQLITE ESTÁNDAR Y ESTABLE)
# ---------------------------------------------------------
DB_FILE = "storia_local.db"
                            ...
                            with get_connection() as conn:
                                ...
                                for _, r in df_pdf.iterrows():
                                    ...
                                    if nom_norm in dict_existentes:
                                        cursor.execute("UPDATE stock SET costo_usd = ?, capacidad_ml = ? WHERE id = ?",
                    ...
                    try:
                        ...
                        if st.button("🚀 Restaurar Copia de Seguridad"):
                            if confirm_restore:
                                with get_connection() as conn:
                                    ...
                                    if "stock" in data_restaurar:
                                        ...
                                        for r in data_restaurar["stock"]:
                                            cursor.execute('''INSERT INTO stock (id, nombre, tipo, genero, capacidad_ml, botellas_100ml_cerradas, ml_disponibles_abiertos, decants_10ml_preparados, costo_usd, margen_100ml_custom, estado, socio_asignado, monto_senado_ars, cliente_senado, notas_olfativas, imagen_url)
                                                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (
                                    ...
                                    if "historial" in data_restaurar:
                                        ...
                                        for r in data_restaurar["historial"]:
                                            cursor.execute('''INSERT INTO historial (id, fecha, perfume, socio, tipo_movimiento, monto_ingreso_ars, id_producto, presentacion, cantidad)
                                                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', (
                                    ...
                                    if "egresos" in data_restaurar:
                                        ...
                                        for r in data_restaurar["egresos"]:
                                            cursor.execute('''INSERT INTO egresos (id, fecha, categoria, descripcion, monto_ars, socio_registra)
                                                              VALUES (?, ?, ?, ?, ?, ?)''', (
                                    ...
                                    if "clientes_seguimiento" in data_restaurar:
                                        cursor.execute("DELETE FROM clientes_seguimiento")
                                        for r in data_restaurar["clientes_seguimiento"]:
                                            cursor.execute('''INSERT INTO clientes_seguimiento (id, fecha_compra, cliente_nombre, cliente_celular, socio_vendedor, perfume, presentacion, dias_estimados, fecha_recordatorio, estado)
                                                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', (
                                ...
                                st.success("🎉 ¡Todos los datos de la copia de seguridad se restauraron localmente sin errores!")
                                st.rerun()
                            else:
                                st.warning("Marca la casilla para confirmar.")
