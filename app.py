...
# ---------------------------------------------------------
# CARGA DE CONFIGURACIÓN Y MENÚ PRINCIPAL
# ---------------------------------------------------------
try:
...
if menu == "Inventario y Stock":
    ...
    if df_stock.empty:
        st.info("No hay perfumes cargados en el inventario actualmente. Puedes agregar uno nuevo abajo.")
    else:
        st.dataframe(df_stock, use_container_width=True)

    st.subheader("➕ Agregar Nuevo Perfume al Stock")
        ...
        if submitted:
            if n_nombre:
                try:
                    ...
                    st.error(f"Error al guardar: Asegúrate de que el nombre no esté repetido. Detalle: {e}")
            else:
                st.warning("Por favor, ingresa al menos el nombre del perfume.")

# ---------------------------------------------------------
# 2. REGISTRAR VENTA / MOVIMIENTO
# ---------------------------------------------------------
elif menu == "Registrar Venta / Movimiento":
    st.header("🛒 Registrar Venta o Movimiento de Stock")
    
    df_stock = fetch_df("SELECT id, nombre, capacidad_ml, botellas_100ml_cerradas, ml_disponibles_abiertos, decants_10ml_preparados, costo_usd FROM stock ORDER BY nombre ASC")
    
    if df_stock.empty:
        st.warning("Primero debes agregar perfumes en la sección 'Inventario y Stock'.")
    else:
        ...
        tipo_movimiento = st.selectbox("Tipo de Movimiento", ["Venta Botella Cerrada", "Venta Decant 10ml", "Fraccionar Botella (Pasar a Abierto)", "Ingreso Manual"])
        socio = st.selectbox("Socio a cargo", ["Socio A", "Socio B", "Ambos / General"])
        
        if "Venta" in tipo_movimiento:
            ...
            st.info("Seleccionaste un movimiento operativo o manual.")
...
# ---------------------------------------------------------
    ...
    if df_hist.empty:
        st.info("El historial de ventas está vacío por el momento.")
    else:
...
# ---------------------------------------------------------
elif menu == "Configuración del Sistema":
    st.header("⚙️ Configuración General")
    st.write(f"Cotización actual del dólar configurada: **${COTIZACION_DOLAR} ARS**")
    st.write(f"Margen 100ml: **{MARGEN_100ML_DEF}%** | Margen Decant: **{MARGEN_DECANT_DEF}%**")
    
    with st.form("form_config"):
        nuevo_dolar = st.number_input("Actualizar Cotización Dólar (ARS)", value=COTIZACION_DOLAR)
        nuevo_m100 = st.number_input("Margen 100ml por defecto (%)", value=MARGEN_100ML_DEF)
        nuevo_mdec = st.number_input("Margen Decant por defecto (%)", value=MARGEN_DECANT_DEF)
